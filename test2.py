...
def main():
    ...
    class Player(pygame.sprite.Sprite):    #spire chỉ đối tượng có thể di chuyển
        ...
        def move_right(self):
            if self.rect.right < width:  # Ensure the player doesn't go out of the screen
                self.rect.move_ip(self.speed, 0)
    ...
    def calc_next_worm_time(max_worm_time, min_worm_time):
    ...
    def run_game():
        ...
        with mp_hands.Hands(max_num_hands=1) as hands:
            nonlocal repeat_game, player_victory, level, level_reset, max_lives, apples_reset, clock_tick_playing, tick_channel, next_apple_time, next_worm_time, next_speed_boost, next_turtle_time, next_extra_jump, next_poison_time, next_extra_lives_time, next_golden_apple
                ...
                while repeat_game:
                    ...
                    while not stop_game:
                        ...
                        if time.time() > next_worm_time:
                            Worm(level)
                            next_worm_time = calc_next_worm_time(max_worm_time, min_worm_time)
                        if level >= 4:
                        ...
                        # Vòng lặp ngoài tiếp tục xử lý hình ảnh từ camera và các hành động khác
